...
public:
	...
	inline double getFuelQtyExternalCenter() const;
	...
	inline void setFuelCapacity(double lo, double co, double ro);
...
private:
	...
	double m_fuel[NUMBER_OF_TANKS] = { 0.0, 0.0, 0.0,}; //vorher waren es 5 jetzt nur 4 (3x external 1x intrenal)

	double m_fuelPrevious[NUMBER_OF_TANKS] = { 0.0, 0.0, 0.0,};

	bool m_fuelEmpty[NUMBER_OF_TANKS] = { false, false, false, false,};

	bool m_fuelSet[NUMBER_OF_TANKS] = { false, false, false, false,}; //Check, if tank is empty or full

	//										   INT    LeftO  CenterO  RightO   
	double m_fuelCapacity[NUMBER_OF_TANKS] = { 5000.0, 800.0, 1200.0, 800.0,}; //values in kg 
	...
	Vec3 m_fuelPos[NUMBER_OF_TANKS] = { Vec3(), Vec3(), Vec3(), Vec3(),}; //hier kommt die Position der Tanks rein, wobei der GENAUE Wert der Lua hier rein müsste, damit die Tanks auf dem CG liegen.
...
void Fuelsystem::setInternal(double value)
{
	...
	//m_fuel[INTERNAL_FRONT] = value; //----das war der alte code
	
	//m_fuelPos[INTERNAL] = Vec3(); auskomentiert, da keine Position übergeben wird von ed_fm_set_internal_fuel !!
}

void Fuelsystem::setFuelCapacity(double lo, double co, double ro)
{
	m_fuelEmpty[LEFT_OUTSIDE] = lo < 0.0;
	m_fuelEmpty[CENTER_OUTSIDE] = co < 0.0;
	...
	if (m_fuelSet[CENTER_OUTSIDE] && co < 0.0)
	{
		m_fuel[CENTER_OUTSIDE] = 0.0;
		m_fuelSet[CENTER_OUTSIDE] = false;
	}
	...
	m_fuelCapacity[CENTER_OUTSIDE] = abs(co);
...
{
	return m_fuel[LEFT_OUTSIDE] + m_fuel[RIGHT_OUTSIDE] + m_fuel[CENTER_OUTSIDE];
}
...
{
	return m_fuel[LEFT_OUTSIDE];
}

double Fuelsystem::getFuelQtyExternalRight() const
{
	return m_fuel[RIGHT_OUTSIDE];
}

double Fuelsystem::getFuelQtyExternalCenter() const
{
	return m_fuel[CENTER_OUTSIDE];
}
...
{
	return 0;//m_fuel[RIGHT_INSIDE] + m_fuel[LEFT_INSIDE];
}

double Fuelsystem::getFuelQtyTotal() const
{
	return m_fuel[RIGHT_OUTSIDE] + m_fuel[LEFT_OUTSIDE] + m_fuel[CENTER_OUTSIDE] + m_fuel[INTERNAL];
}
...
double Fuelsystem::getAdjFuelQtyInternal()
{
	...
	//bei einer Anzeige VOLL = 1.0 und VOLL ist 6.416kg dann ist der Modifikator 1/6416 = 0.00015586
	//Hier muss jetzt also die Anzeige bei 1.0 auf 6416kg oder auch 14.115 lbs gesetzt werden
	m_adjIntFuelQty = getFuelQtyInternal() * 0.00015586;
...
bool Fuelsystem::externalFull() const
{
	return m_fuel[LEFT_OUTSIDE] == m_fuelCapacity[LEFT_OUTSIDE] &&
		m_fuel[CENTER_OUTSIDE] == m_fuelCapacity[CENTER_OUTSIDE] &&
