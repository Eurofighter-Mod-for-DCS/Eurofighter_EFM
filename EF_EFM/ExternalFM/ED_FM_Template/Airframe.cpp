...
void Airframe::airframeUpdate(double dt)
{
	...
	*/
	...
	m_nozzle2Position = setNozzlePosition2(dt);
	...
	//---------Engine flame-out function Engine 2------------------
	if (((getCompressorDamage() < 0.15) || (getTurbineDamage() < 0.15)) && (m_input.getEngineStart2() == 1))
	{
	...
	else if ((getCompressorDamage() >= 0.65) && (getTurbineDamage() >= 0.65) && (m_engine.m_needRestart2 == false))
	{
...
// in Rad = 0.175 und 0.157
	...
	{
		if (m_state.m_aoa >= 0.175)// && ((m_autoDriveLeFlaps == 0.0) || (m_autoDriveLeFlaps == 1.0)))
		{
			m_autoDriveLeFlaps = 1.0;
		}
		else if (((m_state.m_aoa >= 0.157) && (m_state.m_aoa < 0.175)) && (m_autoDriveLeFlaps == 1.0))
		{
			m_autoDriveLeFlaps = 1.0;
		}
		else
...
double Airframe::setNozzlePosition2(double dt) //Nozzle-Position 0-10% Thrust open, 11-84% Thrust closed, 85-100% Thrust open
{
	double NozzlePos = 0.0;
	double corrThrottle = 0.0;
	...
	double input = NozzlePos;
	return m_actuatorNozzle2.inputUpdate(input, dt);
}
